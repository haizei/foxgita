...
struct MigrationTests {
    ...
    @Test func v16StoreMigratesToV17KeepingSubdivisionAndSoundFields() throws {
        let url = FileManager.default.temporaryDirectory
            .appendingPathComponent("gita-v16-v17-\(UUID().uuidString).store")
        defer { try? FileManager.default.removeItem(at: url) }
        ...
        do {
            ...
            let config = ModelConfiguration(schema: v16Schema, url: url)
            let container = try ModelContainer(for: v16Schema, configurations: [config])
            let context = ModelContext(container)
            context.insert(GitaSchemaV16.PracticeItem(
                id: itemId,
                profileId: profileId,
                practiceDayKey: "2026-09-06",
                title: "扫弦",
        ...
        #expect(items.count == 1)
        #expect(items[0].metronomeAccentRaw == "2111")
        #expect(items[0].metronomeSubdivisionRaw == 3)
        #expect(items[0].metronomeSoundModeRaw == nil)
